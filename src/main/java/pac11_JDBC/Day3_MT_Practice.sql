create database day3_mt_practice;
use day3_mt_practice;
/*************************************************************
                        Day3.多表查询练习
-- 需求：
...
from emp as t1
         left join job as t2 on t1.job_id = t2.id;
...
from emp t1
         ...
         left join dept t3 on t1.dept_id = t3.id,
     salarygrade t4
where t1.salary between t4.losalary and t4.hisalary;

-- 5.查询出部门编号、部门名称、部门位置、部门人数
select t1.*, t2.sum
from dept t1,
     (select t.dept_id, count(id) as sum
      from emp t
      group by t.dept_id) t2
